...
function DropBall({ x, height, nonce }: { x: number; height: number; nonce: number }) {
  ...
  useFrame((_, dt) => {
    ...
    m.position.set(x, 8 - (8 - height - 0.25) * e, 0);
...
function Contents({ store }: { store: SceneStore }) {
  ...
  const pendingIdx = rows.findIndex((r) => r.token === pending);
  const maxP = Math.max(...rows.map((r) => r.pTemp), 0.05);
  ...
  return (
    <>
      ...
      {pendingIdx >= 0 && <DropBall x={xOf(pendingIdx)} height={rows[pendingIdx].pTemp * scale} nonce={dropNonce} />}
    </>
  );
}
